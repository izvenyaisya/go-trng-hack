...
Как это реализовано (референс к коду):
- Для каждой новой транзакции создаётся `Block` с полями `Index`, `Timestamp`, `TxID`, `DataHash` (здесь используется `Transaction.Published`), `PrevHash` и `Hash`.
- `computeBlockHash` собирает строку в формате `Index:Timestamp:TxID:DataHash:PrevHash` и вычисляет SHA256 — это значение сохраняется в `Block.Hash`.
- `appendBlock` добавляет новый блок в `chain`, записывает `store.json` и логгирует операцию.
- `validateChain` последовательно проверяет, что для всех блоков `computeBlockHash(block) == block.Hash` и что `PrevHash` каждого блока равен `Hash` предыдущего блока.

Какие гарантии даёт этот подход
- При наличии оригинального `store.json` и неизменного кода вы можете обнаружить локальные модификации: если кто-то изменит поля блока или цепочку, `validateChain()` это выявит (`Hash` перестанет совпадать).
- Поскольку `DataHash` в блоке — это опубликованный отпечаток генерации (`Published`), сама цепочка служит индексом ссылок на содержимое транзакций и их отпечатков.

Ограничения и важные замечания
- Отсутствие распределённого консенсуса: это локальная структура данных — нет репликации, не используется цифровая подпись транзакций и нет защиты от удалённого или локального удаления/перезаписи `store.json`.
- Требуется доверие к хосту: если злоумышленник получит доступ к файловой системе или кода, он может модифицировать `store.json` и бинарь так, чтобы проверки снова проходили (например, изменить и `Hash`, и поля).
- Нет таймстемпинга третьей стороны: хотя `Timestamp` есть, он полагается на локальные часы. Для внешней нереляционной верификации стоит анкеровать (anchor) `Published` в публичном сервисе (например, записать отпечаток в публичный блокчейн, сервис таймстемпов или простую публичную заметку).
- Объём данных: перед сохранением `saveStore()` очищает `SimulationData` в копии транзакции, чтобы не записывать большие пути/рендеры — это делает `store.json` компактнее, но означает, что полные симуляции не всегда хранятся внутри `store.json`.

Рекомендации по использованию
- Используйте `validateChain()` и endpoint `GET /tx/{id}/verify` для локальной проверки целостности.
- Для повышенной надёжности периодически экспортируйте или анкеруйте `Published` в внешнем immutable сервисе (простое решение — публиковать хэши в git commit, записывать в публичный pastebin или отправлять транзакцию в публичный блокчейн).
- Делайте бэкапы `store.json` и храните их в надёжном месте; рассмотреть подпись файла с помощью GPG для детерминированного обнаружения изменений.


Пример: воспроизведение генерации (PowerShell)
...
Инструменты (`tools/`)
- В каталоге `tools/` находятся примеры клиентов/утилит для воспроизведения: `run_generate.go`, `run_generate_info.go`. Они демонстрируют полный цикл: вызвать `/generate`, получить `/tx/{id}/info` и `/tx/{id}/stats`.

Советы и предупреждения
- Для воспроизводимости при использовании `entropy=http` сохраняйте `PerHTTPSeeds` из `/tx/{id}/info` или сохраните `store.json` — это позволит воссоздать TRNG даже если внешние URL изменились или стали недоступны.
- Храните `Provenance.Whiten` с транзакцией — при изменении алгоритма отбеливания старые `BitsHash` перестанут совпадать.

Следующие шаги (при желании):
- Добавить unit-тесты для `deriveSeed` (включая HTTP-mock), `NewTRNGFromTx`, и валидацию `verify` endpoint.
- Экспортировать утилиты `tools/` как отдельные бинарники в Makefile/gh-actions для удобства CI.
